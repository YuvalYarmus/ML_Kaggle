...
print(df.diagnosis.head(20), end="\n\n")
...
X_features = df.columns.difference(['diagnosis'])
...
scaler.fit(df[X_features])
scaled = scaler.transform(df[X_features])
scaled = pd.DataFrame(scaled, columns=X_features)
print(scaled.head(), end="\n\n")

# subplot_lev = 2
# fig, axes = plt.subplots(15, subplot_lev, figsize=(20,20))
# for i in range(15):
#     for j in range(subplot_lev):
#         curr_feature = df.columns[i*subplot_lev + j]
#         if curr_feature == 'diagnosis':
#             sns.countplot(data=df, x=curr_feature, ax=axes[i,j])
#         else:
#             sns.regplot(data=df, x=curr_feature, y='diagnosis', ax=axes[i,j])
